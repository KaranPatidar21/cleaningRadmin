...
function ProductList() {
  ...
  const fetchProducts = async (selected) => {
    ...
    try {
      ...
      const offset = selected * rowsPerPage;

      const response = await getData(
        `api/product/get-all?limit=10&offset=${offset || currentPage}`
      );
      if (response?.success) {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await deleteData(`api/product/delete/${id}`);
        if (response.success) {
        ...
        <div className="container-fluid">
          ...
          <div className="py-3 d-flex align-items-sm-center flex-sm-row flex-column justify-content-between">
            <div className="flex-grow-1">
              <h4 className="fs-18 fw-semibold m-0">Product List</h4>
            </div>
            <div>
              <button
                          ...
                          {products?.length > 0 ? (
                            products?.map((product) => (
                              <tr key={product.product_id}>
                                ...
                                {/* <td>
                                  {product.subcategory_info
                                    ?.subcategory_title || "-"}
                                </td> */}
                                <td>Rs:{product.prev_cost_price || "0.00"}</td>
                                <td>
                                  <span
                                    className={`badge ${
                                      product.is_active
