...
function AddChildCategory() {
  ...
  const getCategories = async () => {
    try {
      const res = await postData("subCategory/findAll")
...
console.log(error)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (editData) {
        response = await postData(`/childCategory/update/${editData._id}`, payload);
      } else {
        response = await postData("/childCategory/create", payload);
      }

      if (response.status) {
        toast.dismiss();
        toast.success(response.message || "Success");
        setTimeout(() => navigate("/childcategory-list"), 3000);
      } else {
        ...
        <div className="container-fluid">
          <div className="py-3 d-flex justify-content-between align-items-center">
            <h4 className="fs-18 fw-semibold m-0">
              {editData ? "Edit Child Category" : "Add New Child Category"}
            </h4>
