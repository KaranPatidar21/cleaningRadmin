...
function ChildCategorylist() {
  ...
  const fetchSubCategories = async () => {
    setLoading(true);
    try {
      const response = await postData("/childCategory/listing", {
        limit: limit,
        page: currentPage + 1,
        search: searchTerm,
      });
    console.log(response.data)
      if (response.status) {
        setSubCategories(response.data.childCategories);
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await postData(`/childCategory/remove`, {
          childCategoryId: id
        });
        if (response.status) {
      ...
      <div className="content">
        <div className="container-fluid">
          ...
          <div className="py-3 d-flex align-items-sm-center flex-sm-row flex-column justify-content-between">
            <h4 className="fs-18 fw-semibold m-0">Child Category List</h4>
            <button
