...
function UpdateWarehouseManagement() {
  const navigate = useNavigate();
  const location = useLocation();
  const warehouseData = location.state?.warehouse; // Get the warehouse data from location state
  ...
  // Populate the form with the warehouse data when the component mounts
  useEffect(() => {
    if (warehouseData) {
      setWarehouse(warehouseData);
      setWarehouseImagePreview(warehouseData.warehouseImage);
    }
  }, [warehouseData]);
  ...
  const handleSubmit = (e) => {
    ...
    const warehouseIndex = existingWarehouses.findIndex(
      (w) => w.warehouseName === warehouseData.warehouseName
    );

    // Update the warehouse data
    if (warehouseIndex !== -1) {
      existingWarehouses[warehouseIndex] = updatedWarehouse;
    }
        ...
        <div className="container-fluid">
          <div className="py-3 d-flex align-items-sm-center flex-sm-row flex-column justify-content-between">
            <div className="flex-grow-1">
              <h4 className="fs-18 fw-semibold m-0">Update Warehouse</h4>
            </div>
            <div>
              <button
                type="button"
...
export default UpdateWarehouseManagement;